...
> 油猴脚本 / Tampermonkey 用户脚本，为 [DeepSeek 开放平台](https://platform.deepseek.com/usage) 用量页补全**今日数据**：今日消费、今日用量、各模型今日/昨日请求次数与缓存命中率，并为官方图表 tooltip 补全每日缓存命中率。注入内容**克隆官方原生卡片样式**，与页面融为一体，以假乱真。
...
- 每个模型的图表卡下方追加 **昨日 / 今日** 请求次数与 **今日缓存命中率** 行
- 官方图表 tooltip 追加当日**缓存命中率**，悬停任意一天即可查看（官方只显示命中/未命中 token，无命中率）
...
### v2.1+（Panorama 时代）
- **注入版 v2.1.1 / 面板版 v2.0.5**：修复 tooltip 缓存命中率计算错误（按当前模型命中/未命中精确计算，此前错用总命中率与错位取值显示约 50% 假值）；修复 v4-pro 加入后模型行（今日/昨日/缓存命中率）全部消失的问题；移除柱状图曲线叠加设计；面板折叠按钮点击区域扩大（UI 不变）
- **注入版 v2.0.9 / 面板版 v2.0.4**：项目更名为 DeepSeek Usage Panorama / Panorama Monitor，@name 与 Greasy Fork slug 同步更新
- **注入版 v2.0.8 / 面板版 v2.0.3**：新增中文脚本名（DeepSeek 用量显示优化 / DeepSeek 用量监控悬浮窗）

### v2.0.x（重构后）
- **v2.0.7**：**回退卡片布局**——注入卡片恢复为紧邻官方锚点卡插入（今日消费→消费金额后、今日用量→Tokens 后），即用户验收通过的原始设计
- **v2.0.6（已回退）**：UI改版，太丑，于 v2.0.7 回退
