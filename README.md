...
> 油猴脚本 / Tampermonkey 用户脚本，为 [DeepSeek 开放平台](https://platform.deepseek.com/usage) 用量页补全**今日数据**：今日消费、今日用量、各模型今日/昨日请求次数与缓存命中率，并在官方柱状图上叠加每日命中率趋势曲线。注入内容**克隆官方原生卡片样式**，与页面融为一体，以假乱真。
...
- 官方 Tokens 柱状图上叠加**每日缓存命中率趋势曲线**（绿色折线+数据点，随窗口缩放自适应）
